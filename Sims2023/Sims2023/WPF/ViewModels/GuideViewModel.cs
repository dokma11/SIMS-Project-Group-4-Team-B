    ...
    public partial class GuideViewModel : IObserver, INotifyPropertyChanged
    {
        ...
        private void CreateVouchersForCancelledTour(int toursId)
        {
            ...
            foreach (var reservation in _tourReservationService.GetAllReservations())
            {
                ...
                {
                    Voucher voucher = new(0,Voucher.VoucherType.CancelingTour, _userService.GetById(reservation.UserId), _tourService.GetById(toursId), DateTime.Now, DateTime.Today.AddYears(1), additionalComment, false);
